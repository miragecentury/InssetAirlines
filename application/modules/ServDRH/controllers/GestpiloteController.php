...
class ServDRH_GestpiloteController extends Zend_Controller_Action
{
    ...
    public function newpiloteAction()
    {
        ...
        if ($this->getRequest()->getParam('enregistrer') == 'Enregistrer') {
             ...
             * en cas d'annulation
             */
            $this->_redirect(Zend_Registry::get('BaseUrl') . '/ServDRH/gestpilote');
        } else {
            /*
                        ...
                        if ($_GET['rep'] == 'oui') {
                            ...
                            if ($employe->get_labelMetier() == 'Pilote') {
                                ...
                                for ($i = 0; $i < sizeof($qualifications); $i++) {
                                    $qualifications[$i]->delete($_GET['id'], $qualifications[$i]->get_Habilitation_noHabilitation());
                                }
                            }
                            //Supprime l'employé
                ...
                if (!empty($conges)) {
                    ...
                    if (isset($_GET['conge']) && is_numeric($_GET['conge'])) {
                        ...
                        if (!is_null($congeSelect)) {
                            if (isset($_GET['action'])) {
                                if ($_GET['action'] == "valid") {
                                    ...
                                    $this->_redirect(Zend_Registry::get('BaseUrl') . '/ServDRH/gestpilote/congespilote?id=' . $_GET['id']);
                                }
                        ...
                        $this->view->erreurIdConge = "<center><p>Id congé invalide</p>
                            Redirection en cours...</center>";
                        $this->getResponse()->setHeader('refresh', '3,url=' . Zend_Registry::get('BaseUrl') .
                            '/ServDRH/gestpilote/congespilote?id=' . $_GET['id']);
                    }
                } else {
            ...
            if (is_numeric($_GET['id']) && is_numeric($_GET['conge'])) {
                ...
                if (!is_null($congeSelect) &&
                    ...
                    if ($this->getRequest()->getParam('enregistrer') == 'Enregistrer') {
                        ...
                        if ($this->getRequest()->isPost()) {
                            ...
                            if ($updForm->isValid($donneesForm)) {
                                ...
                                if (checkdate($donneesForm['moisDateDebut'], $donneesForm['jourDateDebut'], $donneesForm['anneeDateDebut']) == true) {
                                    if (checkdate($donneesForm['moisDateFin'], $donneesForm['jourDateFin'], $donneesForm['anneeDateFin']) == true) {
                                        if (checkdate($donneesForm['moisDateDebutAnnee'], $donneesForm['jourDateDebutAnnee'], $donneesForm['anneeDateDebutAnnee']) == true) {
                    ...
                    $this->getResponse()->setHeader(
                        'refresh', '3,url=' . Zend_Registry::get('BaseUrl') .
                        '/ServDRH/gestpilote');
                }
            }
        }
